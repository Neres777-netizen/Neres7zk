...
-- ⚽ Bndanike Ultra Script
-- Atravessa SOMENTE jogadores (ULTRA)
-- Estável • Anti-void • Botão móvel • Brookhaven
...
title.Text = "⚽ Bndanike Ultra Script"
...
CreateButton("DesarmeAuto", 1)
CreateButton("AntiPulo", 2, function(state)
	if Humanoid then
		Humanoid.JumpPower = state and 0 or 50
	end
end)

CreateButton("Velocidade", 3, function(state)
	if Humanoid then
		Humanoid.WalkSpeed = state and 45 or 16
	end
end)

CreateButton("PingReducer", 4, function(state)
	OptimizePing(state)
end)
